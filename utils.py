...
def variogram_gamv(data, cellsize, nlag, azm, atol, bandh):
    a2g(data, "gamv.dat")
    with open("gamv.par", "w") as f:
        ...
        f.write(str(azm) + " " + str(atol) + " " + str(bandh) + " 0.0  22.5  25.0 -azm,atol,bandh,dip,dtol,bandv \n")
        f.write("0                                       -standardize sills? (0=no, 1=yes)    \n")
        f.write(str(sum(range(1, data.shape[1] - 1))) + " -number of variograms               \n")
    ...
    lag = np.arange(1, int(nlag + 1), dtype = float).reshape((nlag, 1))
    i = -1
    gamma = np.empty((nlag + 2, sum(range(1, data.shape[1] - 1))))
    with open("gamv_out.out") as f:
        for line in f:
            if line[0] == "C":
                i += 1
                n = 0
                ...
                gamma[n, i] = float(g)
                n += 1
    gamma = np.hstack((lag, lag, gamma[2:, :]))
...
def variogram_config(variogram):
    ...
    for dim in range(2, variogram.shape[1]):
        popt, _ = curve_fit(func, variogram[:3, 0], variogram[:3, dim])
        nug[dim - 2] = max(popt[1], 0)
        i = 0
        while (variogram[i + 1, dim] - variogram[i, dim]) / (variogram[1, 0] - variogram[0, 0]) > 0.004 and i < \
                variogram.shape[0]-2:
            i += 1
        max_r[dim - 2] = i + 1
...
def plot_variogram(variogram, name=None, color="green", vmodel=None):
    names = {'1': 'Ag', '2': 'Al', '3': 'Au', '4': 'B',
             ...
             '25': 'Zr'}
    if variogram.shape[1] == 4:
        fig, axs = plt.subplots(1, 2)
        plt.suptitle(name, size = 20)
        axs.set_ylim(0.0, 1.5)
        if len(variogram.shape) == 2:
            axs[0, 0].plot(variogram[:, 0], variogram[:, 2], linestyle = '--', marker = 'x',
                           markersize = 0.5, linewidth = 0.8, color = color)
            axs[0, 0].set_xlabel('Distance')
            axs[0, 0].set_ylabel('Fe')
            axs[0, 1].plot(variogram[:, 0], variogram[:, 3], linestyle = '--', marker = 'x',
                           markersize = 0.5, linewidth = 0.8, color = color)
            axs[0, 1].set_xlabel('Distance')
            axs[0, 1].set_ylabel('Mn')
        else:
            for lines in range(variogram.shape[2]):
                axs[0, 0].plot(variogram[:, 0, lines], variogram[:, 2, lines], linestyle = '--', marker = 'x',
                               markersize = 0.5, linewidth = 0.8, color = color)
                axs[0, 0].set_xlabel('Distance')
                axs[0, 0].set_ylabel('Fe')
                axs[0, 1].plot(variogram[:, 0, lines], variogram[:, 3, lines], linestyle = '--', marker = 'x',
                               markersize = 0.5, linewidth = 0.8, color = color)
                axs[0, 1].set_xlabel('Distance')
                axs[0, 1].set_ylabel('Mn')
    else:
    ...
    if vmodel is not None:
        for i in range(25):
            axs[int(i % 5), int(i / 5)].axvline(vmodel[int((51 - i) * i / 2), 1], linewidth = 0.8, color = 'k', label = 'Samples')
    if name is not None:
...
def sgs(input, if_show, vmodel):
    ...
    for i in range(2, dim):
        ...
        cellsize = 1
        nug = vmodel[int((51 - (i-2)) * (i-2) / 2), 0]
        nst = 1
        it1 = 1
        cc1 = 1 - nug
        azi1 = 0.0
        max_range = vmodel[int((51 - (i-2)) * (i-2) / 2), 1]
