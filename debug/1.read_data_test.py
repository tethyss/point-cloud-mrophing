...
def plot_variogram(variograms):
    ...
    axs[0].plot(variograms[:, 0], variograms[:, 2], linewidth = 1.5, color = 'r')
    axs[0].tick_params(axis = 'both', labelsize = 15)
    axs[0].set_box_aspect(1 / 2)
    axs[0].set_ylim(0, 1.5)
    axs[1].plot(variograms[:, 0], variograms[:, 3], linewidth = 1.5, color = 'r')
    axs[1].tick_params(axis = 'both', labelsize = 15)
    axs[1].set_box_aspect(1 / 2)
    axs[1].set_ylim(-0.75, 0.75)
    axs[2].plot(variograms[:, 0], variograms[:, 4], linewidth = 1.5, color = 'r')
