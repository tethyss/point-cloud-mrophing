...
def search_box(x, pool, knn):
    ...
    r = math.ceil(math.sqrt(knn*density)*10)
    x0 = max(0, int(x[0]-r))
...
if __name__ == '__main__':
    ...
    cProfile.run('TPS(mf_sim, mf_raw, landmarks, landmarks_cdf, rawdata, knn=10, if_show=True, show=[2, 3])')
    result, result_cdf = TPS(mf_sim, mf_raw, landmarks, landmarks_cdf, rawdata
                             , knn=10, if_show=True, show=[2, 3])
pass
