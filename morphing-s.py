...
if __name__ == '__main__':
    ...
    'read origin data'
    rawdata, show, y_label, _ = read_data(test=if_test, sel=False)
    ...
    for epoch in range(epochs):
        ...
        standardizer.fit(landmarks[:, 2:])
        landmarks[:, 2:] = standardizer.transform(landmarks[:, 2:])
        ...
        for r in tqdm(range(mf_repeat), position = 0, leave = True):
            ...
            mf_variogram = variogram_gamv(mf_raw, cellsize = lag, nlag = nlag, azm = 90, atol = 22.5, dbglevel = 0)
        ...
        mf_variogram = variogram_gamv(mf_ave, cellsize = lag, nlag = nlag, azm = 90, atol = 22.5, dbglevel = 0)
