...
if __name__ == '__main__':
    ...
    for i in range(6):
        axs[i // 3, i % 3].imshow(rawdata[:, :, i], cmap = 'jet',origin = 'lower')
        axs[i // 3, i % 3].scatter(landmarks[:, 0], landmarks[:, 1], c = 'none', s = 30, edgecolor = 'grey')
        axs[i // 3, i % 3].set_title('Z' + str(i + 1),size=20)
        axs[i // 3, i % 3].tick_params(axis='both', labelsize=15)
    plt.show()
